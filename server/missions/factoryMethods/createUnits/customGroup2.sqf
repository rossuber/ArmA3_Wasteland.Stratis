...
for "_i" from 1 to _nbUnits do
{
	...
	_dis_chk_group = _pos distance [ 7000, 7000, 0 ];
	_close_sea_group = _dis_chk_group < 600;

	if ( _close_sea_group ) then {
		_spawns_group = nearestObjects [ _pos, [ "LocationRespawnPoint_F" ], 50 ];
		[ _spawns_group, 5 ] call KK_fnc_arrayShuffle;
		_pick_group_pos = _spawns_group call BIS_fnc_selectRandom;
		_uPos = getPosASL _pick_group_pos;
	};

	_unit = _group createUnit [_unitTypes call BIS_fnc_selectRandom, _uPos, [], 0, "Form"];
	if ( _close_sea_group ) then {
		_unit setPosASL _uPos;
	} else {
